import pygame, sys, time, pygame.freetype
import role.god.baseGod, bullet.baseBullet, role.enemy.boss, role.enemy.npc, global_parameter


# 关卡等级1，包括20个小怪，1个boss，boss血量50左右
def Level_1(god=role.god.baseGod.God()):
    pygame.init()
    size = (global_parameter.windows_width, global_parameter.windows_height)
    screen = pygame.display.set_mode(size)
    pygame.display.set_caption('第1关')
    ...
    combat_background = pygame.image.load('Image/background/combat_background1.jpg')
    ...
    boss_attack_judge = 0
    enemy_come = [True, True, True, True, True, True]
    ...
    def combat_information_update(screen):
        ...
        ft.render_to(screen, (1015, 50), '作战信息', fgcolor=global_parameter.BLACK, size=50)
        ft.render_to(screen, (1000, 150), '人物生命:' + str(god.life), fgcolor=global_parameter.BLACK, size=30)
        ft.render_to(screen, (1000, 250), '人物护盾:' + str(god.defendnum), fgcolor=global_parameter.BLACK, size=30)
        ft.render_to(screen, (1000, 350), '小怪数量:' + str(len(enemy_list)), fgcolor=global_parameter.BLACK, size=30)
        ft.render_to(screen, (1000, 450), '可用技能:' + str(god.skill_time), fgcolor=global_parameter.BLACK, size=30)
        if len(boss_list) != 0:
            ft.render_to(screen, (1000, 550), 'BOSS生命:' + str(boss_list[0].life), fgcolor=global_parameter.BLACK,
                         size=30)
    ...
    def boss_come():
        ...
        if boss_come_judge == True and len(enemy_list) == 0:
            boss_list.append(role.enemy.boss.Boss(name="level", life=50, speed=[-5, 0]))  # boss出现
                    ...
                    if god.skill_type == 2:
                        ...
                        for i in god.specialattack(24):
                            god_bullet_list.append(
                                bullet.baseBullet.BaseBullet(speed=[i.speed[0], i.speed[1]], pict=god.blt.pict,
                                                             pos=i.repr.center, camp=i.camp))
